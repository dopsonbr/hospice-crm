        ...
        <CardContent className="flex items-start gap-4 p-4">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-start justify-between gap-2">
              <div>
                <p className="font-medium text-slate-900 dark:text-white">{task.description}</p>
                <div className="mt-1 flex flex-wrap items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
                  {taskType && <span className="flex items-center gap-1">{taskType.label}</span>}
                  {task.facility && (
              ...
              <div className="flex flex-shrink-0 items-center gap-2">
                {priority && (
                  <Badge variant={priorityColors[priority.value]}>{priority.label}</Badge>
                )}
                {task.dueAt && (
                  <span
