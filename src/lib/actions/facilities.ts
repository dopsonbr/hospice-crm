...
export async function getFacilities(): Promise<Facility[]> {
  ...
  return db.select().from(facilities).where(eq(facilities.userId, user.id)).orderBy(desc(facilities.createdAt))
}
...
export async function createFacility(data: Omit<NewFacility, "userId" | "createdAt" | "updatedAt">): Promise<Facility> {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
...
export async function updateFacility(id: string, data: Partial<Omit<NewFacility, "userId" | "createdAt" | "updatedAt">>): Promise<Facility> {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
