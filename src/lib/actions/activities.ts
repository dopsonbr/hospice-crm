...
import { activities, facilities, contacts, deals, type NewActivity, type Activity } from "@/lib/db/schema"
...
export async function createActivity(data: Omit<NewActivity, "userId" | "createdAt">): Promise<Activity> {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
...
export async function updateActivity(id: string, data: Partial<Omit<NewActivity, "userId" | "createdAt">>): Promise<Activity> {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
